...
void MPU6050::calculateOffset(bool isCalculateGyroscope, bool isCalculateAccelerometer) {
    Axis sumAcc;
    Axis sumGy;

    for (int i = 0; i < sampleNumber; i++) { // Leemos los datos del MPU6050 3000 veces y sacamos el valor medio para obtener los offset
        this->readSensors();
        if (isCalculateAccelerometer) {
            sumAcc.x += this->accelerometer.x;
            sumAcc.y += this->accelerometer.y;
            sumAcc.z += this->accelerometer.z;
        }
        if (isCalculateGyroscope) {
...
void MPU6050::calculateAngle() {
    ...
    pitchAngle += rollAngle * sin(recalibrateG.z * executionTime *
                                  (pi / (180 * LSBGyroscope * 1000)));      // tiempo_ejecucion /1000 /65.5 * PI/180
    rollAngle -= pitchAngle * sin(recalibrateG.z * executionTime *
                                  (pi / (180 * LSBGyroscope * 1000)));  //(pi/(180*LSBGyroscope*1000)

    float totalVector = sqrt(pow(accelerometer.x, 2) + pow(accelerometer.y, 2) + pow(accelerometer.z, 2));
    float pitchAngleAcc = asin((float) accelerometer.y / totalVector) *
                          57.2958;     // 57.2958 = Conversion de radianes a grados 180/PI
